<#
.SYNOPSIS
    TidyPackRat - Automated File Management System
...
param(
    ...
    [string]$ConfigPath = "$env:PROGRAMDATA\TidyPackRat\config.json",
...
function Initialize-Logging {
    ...
    try {
        ...
        $logFileName = "TidyPackRat-$(Get-Date -Format 'yyyy-MM').log"
        $script:LogFile = Join-Path -Path $expandedLogPath -ChildPath $logFileName

        # Clean up old log files if there are too many
        $logFiles = Get-ChildItem -Path $expandedLogPath -Filter "TidyPackRat-*.log" |
...
try {
    ...
    if ($config.logging -and $config.logging.enabled) {
        ...
        # Fallback logging
        Initialize-Logging -LogPath "$env:PROGRAMDATA\TidyPackRat\logs"
    }
    ...
    Write-Log -Message "TidyPackRat Worker completed successfully" -Level "SUCCESS"
    exit 0
}
catch {
    Write-Log -Message "TidyPackRat Worker failed: $_" -Level "ERROR"
